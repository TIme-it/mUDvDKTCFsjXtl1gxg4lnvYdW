...
class news extends application_controller {
	...
	public function getPagePidAlias($string) {
		$sql = 'SELECT id FROM news WHERE alias = "'.$string.'"';
		return $this->db->get_one($sql);
	}

	public function getPaged($id) {
		$sql = 'SELECT pid FROM news WHERE id = '.(int)$id;
		return $this->db->get_one($sql);
	}

	public function getOneMY($id){
		$sql = 'SELECT pid, id, MONTH(date) AS month, YEAR(date) AS year FROM news '.
	...
	public function getUrl($id){
		$sql = 'SELECT alias, pid FROM news WHERE id = '.(int)$id;
		return $this->db->get_row($sql);
	}
	...
	public function getAllNews($pid) {
		$sql   = 'SELECT *, MONTH(date) AS month, YEAR(date) AS year FROM news '.
	...
	public function getLast($pid, $limit) {
		$sql = 'SELECT id, pid, title, date, note, source, YEAR(date) as year, MONTH(date) as month FROM news 
			    WHERE pid = '.(int)$pid.' AND active != 0 ORDER BY date DESC, id DESC LIMIT '.(int)$limit;
		return $this->db->get_all($sql);
	}
	
	public function getOneNews($id) {
		$sql = 'SELECT * FROM news WHERE active != 0 AND id = '.(int)$id;
		return $this->db->get_row($sql);
	}
	...
	public function getAboutNewsCategory($id) {
		$sql = 'SELECT title as page_title,alias,note as newsText,template 
				  FROM main 
